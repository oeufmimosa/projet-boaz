...
export default async function SubmissionDetail({
  ...
  if (type === "quote") {
    ...
    return (
      <div>
        ...
        <p className="mt-1 text-sm text-muted-fg">{new Date(q.createdAt).toLocaleString("fr-FR")}</p>
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-muted-fg">{new Date(m.createdAt).toLocaleString("fr-FR")}</p>
      <div className="mt-6 rounded border border-border bg-white p-4">
        ...
        <pre className="mt-4 whitespace-pre-wrap rounded bg-muted p-3 text-sm">{m.message}</pre>
      </div>
    </div>
  );
}
